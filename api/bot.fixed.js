...
async function fetchImageFromUrl(url) {
  ...
  try {
    if (url.startsWith('http')) {
      ...
      // Local file path - use the resolved path
      console.log(`Reading local image file: ${url}`);
...
async function generateReportPdf(data, reportType) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (reportType === 'repair' && TANKER_IMAGE_PATH) {
                console.log(`[PDF Debug] Fetching tanker image from: ${TANKER_IMAGE_PATH}`);
                try {
                    // Use the helper for consistent buffer handling
                    tankerImageBuffer = await fetchImageFromUrl(TANKER_IMAGE_PATH);
